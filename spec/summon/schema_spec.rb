...
describe Summon::Schema do
  ...
  context "boolean values" do
    context "coercion" do
      before do
        class_eval do
          attr :one, :boolean => true, :json_name => "one"
          attr :two, :boolean => true, :json_name => "two"
          attr :three, :boolean => true, :json_name => "three"
        end
      end

      subject { init(:one => ["true"], :two => ["false"], :three => [nil]) }

      context "strings" do
        its(:one) { should eql true }
        its(:two) { should eql false }
      end
      context "nil" do
        its(:three) { should eql false }
      end
    end
    ...
    context "native" do
      ...
      subject { init(:one => [true], :two => [false]) }
      its(:one) { should eql true }
      its(:two) { should eql false }
    end
  end
